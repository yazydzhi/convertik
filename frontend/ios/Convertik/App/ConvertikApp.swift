        ...
        WindowGroup {
            ContentView()
                ...
                .onAppear {
                    ...
                    // 3. ⏳ Синхронизация данных запускается из RatesRepository через 3 секунды
                    // 4. ⏳ Проверяем подписку через 4 секунды (после синхронизации)
                    // 5. ⏳ Инициализируем AdMob через 5 секунд (после premium check)
                    
                    // Проверяем статус подписки тихо без принудительной авторизации
                    // Делаем это ПОСЛЕ синхронизации данных, чтобы не блокировать UI
                    Task.detached {
                    ...
                    // Делаем это ПОСЛЕ проверки premium статуса
                    Task.detached {
