...
extension Date {
    func formattedForDisplay() -> String {
        ...
        // Явно устанавливаем локальный часовой пояс устройства
        // Это гарантирует, что время всегда отображается в часовом поясе пользователя
...
final class RatesRepository: ObservableObject {
    ...
    @Published var lastUpdated: Date? {
        didSet {
            // Сохраняем время последнего обмена в UserDefaults для сохранения между запусками
            if let lastUpdated = lastUpdated {
                UserDefaults.standard.set(lastUpdated, forKey: "lastSyncTime")
            } else {
    ...
    private init() {
        ...
        // Восстанавливаем время последнего обмена из UserDefaults
        if let savedSyncTime = UserDefaults.standard.object(forKey: "lastSyncTime") as? Date {
            self.lastUpdated = savedSyncTime
            logger.debug("Restored lastSyncTime from UserDefaults: \(savedSyncTime.formattedForDisplay())")
        }
    ...
    func loadLocalRates() {
        ...
        do {
            ...
            // Не обновляем lastUpdated при загрузке из локального хранилища
            // lastUpdated обновляется только при успешном обмене данными с сервером
            // и содержит время последнего обмена в локальном часовом поясе клиента
            logger.debug("loadLocalRates: loaded \(entities.count) rates, lastUpdated remains: \(self.lastUpdated?.formattedForDisplay() ?? "nil")")
        } catch {
    ...
    func syncRemote() async {
        ...
        do {
            ...
            // response.updatedAt - техническая информация о времени обновления на сервере
            // Не используется для отображения пользователю
    ...
    private func updateLocalRates(from response: RatesResponse, names: [String: String]) async {
        ...
        await MainActor.run { [weak self] in
            ...
            self.loadLocalRates()
            // Используем текущее локальное время клиента как время последнего обмена данными
            // Это время отображается пользователю в его часовом поясе
            // response.updatedAt - техническая информация о времени обновления на сервере, не отображается
            let syncTime = Date()
            self.lastUpdated = syncTime
            self.logger.debug("Last sync time (local): \(syncTime.formattedForDisplay()), server updated at: \(response.updatedAt.formattedForDisplay()) (technical)")
        }
    }
