"""Add name field to rates table

Revision ID: add_name_field_to_rates
Revises: 633031fc1430
...
revision: str = 'add_name_field_to_rates'
down_revision: Union[str, Sequence[str], None] = '633031fc1430'
...
def upgrade() -> None:
    """Upgrade schema."""
    # Добавляем поле name в таблицу rates
    op.add_column('rates', sa.Column('name', sa.String(length=100), nullable=True))
    
    # Обновляем существующие записи с названиями валют
    currency_names = {
    ...
    for code, name in currency_names.items():
        op.execute(f"UPDATE rates SET name = '{name}' WHERE code = '{code}'")
    
    # Делаем поле name обязательным
    op.alter_column('rates', 'name', nullable=False)
